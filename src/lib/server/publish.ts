...
export async function startPublish(
  ...
  if (getTrackedCommandState(trackingKey)?.status === 'running') return { ok: false }
  ...
  emitter.on('exit', exit => {
    void (async () => {
      try {
        try {
          await finishPublish(project, kind, plan, exit)
        } finally {
...
async function finishPublish(project: Project, kind: string, plan: BuildPlan, exit: TrackedCommandExit) {
  const stagingDir = getPublishStagingDir(project.id, kind)
  try {
    ...
    const id = (await db.publication.findUnique({ where }))?.id ?? crypto.randomUUID()
